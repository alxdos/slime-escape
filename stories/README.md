...
| [011-content-from-md.md](011-content-from-md.md) | [/] | Геймдизайнер правит баланс слаймов в одном MD-файле и одной командой пересобирает конфиг; для игрока поведение не меняется до первого редактирования | `content/enemies.md` как источник правды, генератор в `scripts/content-build/`, пары `<area>.ts ↔ <area>.generated.ts` в `src/shared/content/**` и `src/main/audio/**`, атомарная запись + `content:check` в `npm run build` |
