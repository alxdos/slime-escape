...
| [005-drops.md](005-drops.md) | [/] | С убитых врагов выпадает дроп, игрок подбирает и видит эффект | `Drop` как сущность, `DropArchetype`/`DropEffect`, `EnemyArchetype.dropTable`, `DropSystem` (death hook + ttl/pickup), `dropSpawn`/`dropPickup`/`dropExpire` events |
