...
| [003-combat-foundation.md](003-combat-foundation.md) | [/] | Стрельба по тренировочной мишени, урон и смерть врага | `EnemyArchetype`/`WeaponArchetype` + `Loadout`, `SpawnPlan` static, `SpawnSystem`, `CombatSystem`, `HealthDeathSystem`, `SpatialIndex`, расширение снапшота и `fire`/`hit`/`death` events |
