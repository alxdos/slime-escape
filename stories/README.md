...
| [004-waves-and-zone.md](004-waves-and-zone.md) | [/] | Тренировочный режим из 1–2 волн с передышками, тёмная зона сжимается в волне и расширяется в передышке, win/loss | `SpawnPlan` `'wave'`, `ZoneSystem`, encounter transitions и `win`/`loss` в `SessionFlowSystem`, contact damage и `HasHealth` у игрока, session RNG |
