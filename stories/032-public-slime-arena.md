...
| T19 | [ ] | Restore standard desktop combat affordances in online play. | Public Arena online should reuse the standard necessary HUD affordances instead of feeling bare: bottom-left `WASD`, bottom-right left-click/fire hint, and the standard bottom-center weapon slot bar fed by the generated `portal` session loadout. The first slice content has exactly one selected rock slot (`rock-thrower`), so no extra online weapon switching behavior is added. Regular player movement/health must come from the generated `portal` player, and rock weapon timing/projectile values must come from `content/weapons.md`. Also restyle `PublicArenaMenuOverlay` to match the standard pause/menu contrast and avoid the current black text treatment. Prefer reusing existing HUD/menu subcomponents or narrow online adapters over duplicating bespoke DOM. Update desktop/mobile visibility tests so mobile keeps mobile controls while desktop gets the standard hints. |
