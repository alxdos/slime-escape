...
| T24b | [ ] | Keep virtual aim viewport-relative across local and online camera movement. | Input controllers must store aim as a visible-area offset, convert to world coordinates from the current visible area for rendering and outgoing `aim`, and resync aim when camera movement changes that world coordinate. |
