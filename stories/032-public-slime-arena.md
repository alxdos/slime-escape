...
| T14 | [ ] | Correct T13: replace the hardcoded Public Arena config with generated `portal.md` content. | Remove the hand-written `src/shared/publicArenaConfig.ts` path introduced by T13. Migrate session content from `arenaId` to universal `arenaWidth`/`arenaHeight` fields: all existing non-portal session MD files use `32`/`18`, and `content/sessions/portal.md` uses `40`/`40`. Update the generator so `SessionDefinition.arena` is built from those fields. Update `portal.md` so `/portal` is the online Public Arena config source: it keeps the portal background and removes `portal-wave-*`, `portal-pre-boss-break`, `portal-boss`, local loadout/win/loss flow, zone behavior, rewards, and authored local-session progression. Server bounds, online renderer background, and tests must read the generated shared content projection from the portal content instead of duplicating `portal`, `/images/bg/bg-01.jpg`, or `40`. |
| T15 | [ ] | Correct T10: replace immediate multiplayer exit with a minimal Public Arena menu. | Do not close the Public Arena socket directly from desktop `Esc`, `Space`, Pointer Lock loss, or the mobile menu button. Those inputs should open a small online-only menu overlay with at least one explicit action, `Exit Arena`; choosing it closes the socket and returns to the main menu. This overlay must not call `SimWorkerHost.pause()`, must not reuse local-session pause state as gameplay authority, and may offer a simple resume/dismiss path if needed for desktop Pointer Lock recovery. Update [main-ui-shell.md](../design/main-ui-shell.md), `UiShell`, mobile controls wiring, and tests so accidental key presses do not leave multiplayer immediately. |
| T16 | [ ] | Adjust the Public Arena HUD labels. | Show online population as a simple online count without `/200` (for example, `Online 37`). Show the player's progress as current level out of the final boss level (for example, `Level 3/6`, capped at the boss level if internal kills keep increasing after boss). Update `PublicArenaHud` and `UiShell` tests/harnesses. |
| T17 | [ ] | Address remaining code-review hardening before live verification. | Add a simple per-socket input rate limit for `publicArena:input`, send `closeReason: serverShutdown` before intentional server close, and make projectile hit selection deterministic by choosing the nearest overlapping target instead of `Map` insertion order. Keep this task narrow: no auth, moderation, spatial grid, texture-cache refactor, or broader performance work in this slice. |
