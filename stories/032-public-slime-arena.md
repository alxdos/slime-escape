...
| T18 | [x] | Expand Public Arena progression to the full authored enemy slime roster. | Generated enemy content now exports `ENEMY_ARCHETYPE_LIST` in source order. Public Arena progression derives its slime chain, regular-form stats, and boss-level denominator from that list; server stats use those generated values and no longer fall back to the first slime. Added shared progression and final-slime-before-boss tests. |
| T19 | [ ] | Restore standard desktop combat affordances in online play. | Public Arena online should reuse the standard necessary HUD affordances instead of feeling bare: bottom-left `WASD`, bottom-right left-click/fire hint, and the standard bottom-center weapon slot bar fed by the online loadout. The first slice loadout contains exactly one selected rock slot (`rock-thrower`), so no extra online weapon switching behavior is added. Also restyle `PublicArenaMenuOverlay` to match the standard pause/menu contrast and avoid the current black text treatment. Prefer reusing existing HUD/menu subcomponents or narrow online adapters over duplicating bespoke DOM. Update desktop/mobile visibility tests so mobile keeps mobile controls while desktop gets the standard hints. |
