...
Follow-up for online hit/death audio: when the local player hits another player, only the local player hears the target form's configured hit sound. When any player dies, every connected player hears the dead form's configured death sound. Public Arena presentation events carry the owner/form data needed to reuse existing enemy/boss audio mappings; do not add arena-specific hit/death sample ids.
