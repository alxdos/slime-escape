...
- Status: done
- Created: 2026-04-30
- Updated: 2026-05-01 (closed: T2-T6 implemented and accepted; T7 verified — 974 automated tests green and live online verification by the user with two clients confirmed end-to-end (join → lobby → start → co-op run → ghost-and-revive cycle → PvP slime/boss kill-chain regression check → result → leave → room destroyed). Public Arena PvP audio surface restored after fixing two coupled regressions: `Audio.handleEvent` now routes `kind: 'player'` hit/death through `formArchetypeId`-based enemy/boss sample resolution (per the new rule recorded in [audio.md](../design/audio.md)), and the per-frame `audio.update` in `UiShell.onFrame` now uses the active online snapshot pair instead of the empty local pair so the resolver sees PvP entities. One known follow-up tech debt: `audio.update` is called from two sites (per-snapshot callback and per-frame loop) and the per-frame call previously raced the per-snapshot one; consolidate to a single call site in a future cleanup. Earlier: 2026-05-01 architect prep T1 landed: split companion config from session-level into `PlayerConfig.companion` (per-player); implemented `lossCondition: 'allPlayersDead'` semantics with player ghost-state path; introduced `playerCoopRevive` for symmetric player-rescue; added `SessionRules.playerVsPlayerDamage`; renamed `online-arena-hosting.md` → `online-session-hosting.md` and expanded for multi-room hosting and companion delivery on join; created new `online-lobby.md` for host-controlled-lobby state machine, election, and wire protocol; added `onlineLobby` phase to `main-ui-shell.md` and made the online catalog data-driven; extended `content-authoring.md` for the new session-content fields. Tasks decomposed into T1 (architect, landed), T2/T3 (engine), T4 (content), T5 (host: lobby + multi-room), T6 (main UI: catalog + lobby + co-op HUD + online result), T7 (verify).)
