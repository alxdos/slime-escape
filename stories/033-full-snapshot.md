...
| T2 | [x] | Update `src/shared/publicArenaProtocol.ts`: drop `selfId` and `arena` from `PublicArenaSnapshot`, bump `ARENA_HOST_PROTOCOL_VERSION`, update `publicArenaProtocol.test.ts`. | Protocol test passes. |
