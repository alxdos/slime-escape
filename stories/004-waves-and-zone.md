...
| T3 | [ ] | Контент: новые архетипы врагов в `src/shared/content/enemies.ts` — `slime-fast` (быстрый, мало HP, средний contact damage) и `slime-tank` (медленный, больше HP, выше contact damage); `behavior: 'chase'`, валидные `maxSpeed`/`contactDamage`/`contactCooldownMs`. `PlayerSpawn` (в `players.ts`) получает `maxHp`. Соблюсти ограничения «touring через игрока» и `'stationary' ⇒ maxSpeed === 0 && contactDamage === 0` на стороне content/builder с warning через `log.warn`. | опоры: `design/content-archetypes.md`, `design/enemy-contact.md`, `design/logging.md` |
...
| T5 | [ ] | `EntityStore`: `Player` получает `hp`/`maxHp`, инициализируемые из `SessionDefinition.player.maxHp`; `Enemy` получает `maxSpeed`, `contactDamage`, `contactCooldownMs`, `nextContactSimMs`. `MovementSystem` поддерживает `behavior: 'chase'` (двигаться к позиции игрока с `maxSpeed`, без clamp границами арены для врагов); для `'stationary'` поведение прежнее. Толерантность к `player === null`: `MovementSystem` без player — no-op. | опоры: `design/runtime-systems.md`, `design/health-and-death.md`, `design/enemy-contact.md`, `design/arena-and-coordinates.md` |
...
| T7 | [ ] | `CombatSystem`: новая фаза contact intents между `lifetime cleanup` и `hit detection` по `design/enemy-contact.md`. SpatialIndex как акселератор; circle-vs-circle overlap player↔enemy; per-enemy `nextContactSimMs`; формирование `DamageIntent` с `source.kind: 'enemyContact', enemyId`; единый список intents за тик. Толерантность к `player === null` (фаза no-op). Никаких новых runtime events. | опоры: `design/enemy-contact.md`, `design/projectiles-and-combat.md`, `design/runtime-systems.md` |
...
| T13 | [ ] | Тесты под контракт по `design/testing.md`: `SpawnSystem` для `'wave'` соблюдает `spawnIntervalMs` и `maxAlive`; завершение wave-encounter по `dispatched == total && alive == 0`; линейная интерполяция `margin` за `durationMs` и clamp; transition `wave → break → wave → win` на synthetic-clock; `loss` ровно один раз при playerDeath; повторный intent в умершего игрока не публикует второй `loss`; одинаковый `seed` → одинаковый список `(archetypeId, position)` спавнов. | опоры: `design/testing.md`, `design/spawn-plan.md`, `design/zone.md`, `design/health-and-death.md`, `design/rng.md` |
