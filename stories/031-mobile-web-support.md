...
| T12 | [x] | Fix mobile touch coordinate mapping across the rotated game root: pause/weapon hit tests may use visual client rects, but fire/move/aim zone classification and drag deltas must be converted into the landscape game-surface coordinate frame and must not mix visual `getBoundingClientRect()` coordinates with layout `clientWidth/clientHeight`. | Current implementation fixed child-target `offsetX/Y` only. Add regressions for both child targets and a portrait-rotated surface where `clientWidth/clientHeight` are `844 x 390`, `getBoundingClientRect()` is `390 x 844`, and a tap on the visible movement-stick area classifies as move, not fire. |
