...
- Видит: Settings overlay не как тёмную системную карточку, а как рисованный UI приложения: светлая бумага, яркие элементы, чёрная рамка и резкая тень.
...
- Полный редизайн Pause, Result, combat HUD или wave title overlay за пределами общей comic-типографики. Settings overlay в этой истории получает визуальную оболочку, чтобы не выбиваться из нового главного меню.
...
- Hover любой активной кнопки добавляет читаемую подсветку через glow/brightness без грязной чёрной hover-тени и не меняет её layout footprint.
...
| T7 | [x] | Реализовать menu/startup visual states: appear opacity/scale, active hover/focus highlight, selected difficulty glow + breathing, disabled/soon state, comic text style and reduced-motion fallback. | Motion must not resize layout footprint; font loading is progressive enhancement. |
