...
- Status: done
- Created: 2026-04-23
- Updated: 2026-04-23 (review: `Drops` теперь допускает несколько строк на enemy и ловит duplicate пары; `scripts/**` покрыты `tsconfig.scripts.json`; generator CLI фиксирует repo root)
...
- Существующие npm-скрипты сохраняются по форме; `build` расширяется композицией: `npm run content:check && tsc -p tsconfig.json && tsc -p tsconfig.worker.json && tsc -p tsconfig.scripts.json && vite build`. Vercel-деплой использует тот же `build`, поэтому дрейф ловится на стороне CI без отдельной конфигурации.
