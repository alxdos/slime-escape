...
- Status: done
- Created: 2026-04-23
- Updated: 2026-04-23 (review: `build` сначала проверяет `scripts/**` через `tsconfig.scripts.json`, затем запускает `content:check`)
...
- Существующие npm-скрипты сохраняются по форме; `build` расширяется композицией: `tsc -p tsconfig.scripts.json && npm run content:check && tsc -p tsconfig.json && tsc -p tsconfig.worker.json && vite build`. Vercel-деплой использует тот же `build`, поэтому сначала проверяется генератор, а затем дрейф ловится на стороне CI без отдельной конфигурации.
