...
- Status: done
- Created: 2026-04-24
- Updated: 2026-04-24 (architecture cleanup: drop magnet binding via the new `DropEffect.kind: 'pickupModifier'` made explicit; aim assist owner fixed to the `main thread`; tasks updated.)
...
- Add runtime ownership for the new effects without moving HP decrement out of `HealthDeathSystem` or the drop lifecycle out of `DropSystem`.
...
- Existing sessions without these features still run with unchanged behaviour.
...
| ID | Status | Task | Note |
|----|--------|------|------|
| T1 | [x] | Add content/session types and builders for opt-in field effects, status applications, mine triggers, carrier markers, drop magnet, retaliation policy, and aim assist. | Touch `src/shared/content/**`, `scripts/content-build/**`, `content/{weapons,drops,enemies,sessions}/**`; extend the `DropEffect` union with `kind: 'pickupModifier'` per `design/drops.md` as the single binding point for the drop magnet; validate all cross-area references. |
...
| T4 | [x] | Extend `CombatSystem` with mine proximity triggers and explosion-spawned field effects/status applications. | Reuse universal projectile/explosion ownership and shared damage rules; proximity checks use `SpatialIndex` and deterministic ordering. |
| T5 | [x] | Extend `DropSystem`, enemy content, and the renderer for carrier drops and magnet attraction. | Carrier rewards still use death hooks; magnet movement/pickup expansion stays deterministic and owned by `DropSystem`. |
| T6 | [x] | Add friendly-fire retaliation behaviour and optional aim-assist targeting. | Add aggro memory/behaviour consumption in sim; implement aim assist on the `main thread` per `design/combat-modifiers-and-field-effects.md` (it corrects `aim` before sending the existing `InputCommand`, sim only sees a regular `aim` value); use deterministic tie-breakers for target choice. |
