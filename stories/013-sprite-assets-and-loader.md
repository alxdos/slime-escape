...
- Status: in-progress
- Created: 2026-04-23
- Updated: 2026-04-23 (architect pass: добавлены `design/sprite-assets.md` и расширения `content-authoring.md`/`main-ui-shell.md`; после ручной проверки добавлен follow-up контракт `design/body-contact-boxes.md` для body-contact по sprite-derived `contactBox`)
...
- Симуляция продолжает работать с gameplay-полями (`radius`, `maxHp`, `speed`, damage) и не читает visual registry напрямую. После [../design/body-contact-boxes.md](../design/body-contact-boxes.md) body-contact для `player` / `enemy` / `boss` опирается на derive `contactBox`, а не на `radius`; `radius` остаётся только у не-мигрированных consumers до отдельного follow-up.
- После ручной visual QA выяснилось, что circle body-contact не совпадает с прямоугольными sprite bodies. Follow-up контракт — [../design/body-contact-boxes.md](../design/body-contact-boxes.md): для `player` / `enemy` / `boss` в shared content и runtime появляется derive `contactBox`, равный sprite `worldSize`; `CombatSystem` переводит body-contact на box-vs-box, а `MovementSystem` clamp-ит игрока по box, не по `radius`.
...
| T16b | [ ] | Body-contact collision follow-up: derive `contactBox` в shared content для `players` / `enemies` / `bosses` из того же PNG scale pipeline, что и visual registry; перенести `CombatSystem` contact overlap на box-vs-box; player clamp в `MovementSystem` перевести на `contactBox`; добавить/обновить tests на `content-build`, `CombatSystem`, `MovementSystem`, `buildSession` и live registries. | опоры: `design/body-contact-boxes.md`, `design/content-archetypes.md`, `design/enemy-contact.md`, `design/session-definition.md` |
