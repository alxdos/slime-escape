...
| T9 | [x] | Авторинг `content/players.md`: партиция `# Players` с `## hero` (`field | value` для `displayName`); партиция `# Balance` с `## Body` (`id | radius`), `## Movement` (`id | maxSpeed`), `## Health` (`id | maxHp`), `## Visual` (`id | image`). После сходимости — финальный AUTO-GENERATED. | опоры: `design/content-authoring.md`, `design/sprite-assets.md` |
