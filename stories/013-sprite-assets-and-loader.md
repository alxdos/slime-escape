...
- Архитектурный контракт visual-слоя — [../design/sprite-assets.md](../design/sprite-assets.md): один общий тип `SpriteVisualSpec`, три раздельных visual registry рядом с renderer (`playerVisuals.{ts,generated.ts}`, `enemyVisuals.{ts,generated.ts}`, `bossVisuals.{ts,generated.ts}`), единственная константа `PX_PER_WU = 200` в `src/shared/sprite/spriteScale.ts`.
...
| ID | Status | Task | Note |
|----|--------|------|------|
| T1 | [x] | Архитектор: новый `design/sprite-assets.md` (visual registry, `SpriteVisualSpec`, `PX_PER_WU = 200`, asset-only renderer, hard-error policy, preload contract). Поднять `Updated` в `design/content-archetypes.md` (`color` теряет потребителя в renderer для player/enemy/boss). Обновить `Index` в `design/README.md`. | архитектор |
...
| T5 | [x] | Common visual типы и константа: `src/main/render/SpriteVisualSpec.ts` (тип `SpriteVisualSpec`), `src/shared/sprite/spriteScale.ts` (`export const PX_PER_WU = 200` — единственный источник scale, проверка grep-тестом). Без area-кода. | опоры: `design/sprite-assets.md` |
