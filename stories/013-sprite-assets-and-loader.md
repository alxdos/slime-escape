...
- Status: in-progress
- Created: 2026-04-23
- Updated: 2026-04-23 (architect pass: добавлены `design/sprite-assets.md` и расширения `content-authoring.md`/`main-ui-shell.md`; декомпозиция `Tasks` переведена на новый набор; зависимость от завершённой 012 закреплена)
...
- Архитектурный контракт visual-слоя — [../design/sprite-assets.md](../design/sprite-assets.md): один общий тип `SpriteVisualSpec`, три раздельных visual registry рядом с renderer (`playerVisuals.{ts,generated.ts}`, `enemyVisuals.{ts,generated.ts}`, `bossVisuals.{ts,generated.ts}`), единственная константа `PX_PER_WU = 70` в `src/main/render/spriteScale.ts`. `2240x1260` упоминается только как объяснение, откуда взялось 70; в коде/тестах/MD оно не появляется.
...
- Симуляция продолжает работать с gameplay-полями (`radius`, `maxHp`, `speed`, damage) и не читает PNG. `radius` остаётся collision-параметром: визуальный размер спрайта (`worldSize`) и хитбокс (`radius`) могут отличаться.
...
- Заводится новая контентная область **players** с собственным `content/players.md`: visual id `hero` плюс gameplay-поля игрока (`radius`, `maxSpeed`, `maxHp`). Это первое включение области `players` в MD-конвейер; остальные «session-композиционные» области (`sessions`, `arenas`, `presets`) — по-прежнему out-of-scope.
...
| ID | Status | Task | Note |
|----|--------|------|------|
| T1 | [x] | Архитектор: новый `design/sprite-assets.md` (visual registry, `SpriteVisualSpec`, `PX_PER_WU = 70`, asset-only renderer, hard-error policy, preload contract). Поднять `Updated` в `design/content-archetypes.md` (`color` теряет потребителя в renderer для player/enemy/boss). Обновить `Index` в `design/README.md`. | архитектор |
