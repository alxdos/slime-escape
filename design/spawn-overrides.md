...
- `weapons: WeaponInstance[]` и `selectedWeaponIndex: number | null` (story 020) — материализованы из `override.loadout`, если задан, иначе `weapons: []`/`selectedWeaponIndex: null`. `WeaponInstance` инициализируется так же, как для игрока ([universal-weapons-and-projectiles.md](universal-weapons-and-projectiles.md)): `archetypeId` копируется из `loadout.weapons[i]`, `modifiers: []`, `overdriveUntilSimMs: null`, `nextFireSimMs` — см. [non-player-firing.md](non-player-firing.md). После материализации эти два поля принадлежат runtime-сущности `enemy` и живут до её смерти.
