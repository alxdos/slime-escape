...
- Multi-actor sessions aggregate. Across `SessionDefinition.players[]`, `kills.total`, `kills.byArchetype`, `drops.pickedUpTotal`, `boss`, and `dungeon.wavesCleared` aggregate into one shared shape — there is **no per-actor split** in `SessionResultSummary` for this horizon. Companion-caused hostiles still count, by the existing rule above. `defeat` is recorded on the **first** player death (any of `players[]`) for `lossCondition: 'playerDeath'`; later player deaths during the same tick do not overwrite it. For sessions with `players.length === 1` this is identical to the previous single-player rule. The summary shape and `defeat` semantics for the two new lossCondition kinds recorded in [session-definition.md](session-definition.md) — `'allPlayersDead'` and `'respawnOnDeath'` — are explicitly deferred to their consumer stories (037 and 036). If a future product story needs per-actor stats (e.g. per-player kills in co-op result), that is a new decision against this file, not a hidden extension.
