...
- Status: accepted
- Created: 2026-04-26
- Updated: 2026-05-01 (story 037 prep: implemented `defeat`/summary policy for `lossCondition: 'allPlayersDead'`. `defeat.cause` is the cause of the **last** player to fall — i.e. the cause taken from the `DeathContext` of the player ghost transition that brought the live-actor count to zero. Rationale: in co-op the run ends because the last living actor falls; presentation should describe that final blow on the result screen for that actor, not the first player who fell minutes earlier. The "first player overwrite" rule from `'playerDeath'` does not apply here. Aggregated kills/drops/boss/dungeon stats keep aggregating across `players[]` as recorded for 035 (no per-actor split). For revives that bring a ghost player back to alive (`SessionDefinition.playerCoopRevive`, [session-definition.md](session-definition.md)), `RunSummaryTracker` keeps the previous "last to fall" cause as a candidate `defeat.cause`; if the run continues and another fall later seals it, the candidate is overwritten by that fall's cause. The candidate is never written to `SessionResultSummary.defeat` until `loss` actually fires; if `win` fires first, `defeat` stays `null`. The `'respawnOnDeath'` semantics remain as recorded for 036 — there is no `loss`, so no `defeat`. Earlier: 2026-04-30 story 035 prep: recorded multi-actor `RunSummaryTracker` semantics — kill counts, drops picked up, and boss state aggregate across all `players[]`; `SessionResultSummary` gains no per-actor fields and stays single-shape. `defeat.cause` is the cause of the **first** player to die under `lossCondition: 'playerDeath'`. The two new lossCondition kinds recorded in [session-definition.md](session-definition.md) — `'allPlayersDead'` and `'respawnOnDeath'` — are not implemented in 035; their `defeat`/summary policy is deferred to their consumer stories (037 and 036). Earlier: 2026-04-29 story 030 prep: companion-caused enemy/boss kills count like other defeated hostiles, while companion downed events do not affect result summary; projectile/explosion owner metadata includes `companion`. Earlier: 2026-04-28 story 029 prep: XP result rewards are derived on the main thread from `summary.kills.total`; `SessionResultSummary` shape does not gain XP fields. Earlier: story 028 prep: `SessionResultSummary` gains an optional Dungeon block with fully cleared waves; endless Dungeon uses `progress.percent = null` and does not drive campaign Escape Path. Earlier: 2026-04-27 story 027 prep: loss-only Restart is an orchestration/UI action and does not change the `SessionResultSummary` payload. Earlier: story 026 prep: `portal` encounters are non-objective presentation encounters and do not contribute to result progress; the `/portal` flow can also finish through the normal post-boss win result.)
...
- Multi-actor sessions aggregate. Across `SessionDefinition.players[]`, `kills.total`, `kills.byArchetype`, `drops.pickedUpTotal`, `boss`, and `dungeon.wavesCleared` aggregate into one shared shape — there is **no per-actor split** in `SessionResultSummary` for this horizon. Companion-caused hostiles still count, by the existing rule above. `defeat` recording differs by lossCondition:
  - `'playerDeath'`: recorded on the **first** player death (any of `players[]`); later deaths during the same tick or in following ticks do not overwrite it. For sessions with `players.length === 1` this is identical to the previous single-player rule.
  - `'allPlayersDead'` (story 037): recorded on the **last** player to fall — i.e. the ghost transition that brought the live-actor count to zero. Earlier ghost transitions update a `RunSummaryTracker` candidate but the candidate is overwritten by every subsequent ghost transition while at least one actor remains alive; only the final one is committed when `loss` fires. Revives between transitions do not commit a `defeat`; the candidate persists across revive cycles. If a `win` event ends the run instead, `defeat` stays `null`.
  - `'respawnOnDeath'` (story 036): no `loss` event ever fires for player death — `defeat` is `null` by construction.
  - `'none'`: no automatic terminal events; `defeat` is `null` if a manual `stopSession` triggers presentation, but Result UI ([main-ui-shell.md](main-ui-shell.md)) is only entered on terminal `win`/`loss`, so `defeat` is not surfaced in this case.

  If a future product story needs per-actor stats (e.g. per-player kills in co-op result), that is a new decision against this file, not a hidden extension.
