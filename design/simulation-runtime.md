...
- Two hosts run this core:
  - **Browser simulation worker host** (browser context, used for all local play). Owns clock pumping, message-port input intake, snapshot/event delivery to `main`. Used by campaign, training, dungeon, and any future single-player or local content. Lives under `src/main/**` per [thread-model.md](thread-model.md).
...
- The current `server/src/arenaSimulation.ts` is provisional. It is the first-slice expedient delivered by story 032 and is scheduled for removal once the Node arena host hosts the Public Arena via the shared core. The provisional decision file [public-multiplayer-arena.md](public-multiplayer-arena.md) is removed at the same time, replaced by a focused `online-arena-hosting.md` covering the Node host contract. Until that migration completes, both code paths coexist; new online behavior must not be added to `server/src/arenaSimulation.ts` unless the cost of that addition is also planned for the migration.
- Concrete contracts deferred to subsequent decisions, recorded as their stories enter work:
  - extracting the core from the browser worker host (separating systems and tick from `self.postMessage`/`self.addEventListener`);
...
- The existing decision in [public-multiplayer-arena.md](public-multiplayer-arena.md) describes the first-slice expedient and is scheduled for removal once the Node host hosts the Public Arena; its replacement is a new `online-arena-hosting.md` decision focused on the Node host contract.
