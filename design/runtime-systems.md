...
- Dynamic roster (story 036, [sim-core-interface.md](sim-core-interface.md), [session-definition.md](session-definition.md)):
  ...
  - `addPlayer.opts.invulnerableUntilSimMs`, when set, becomes a runtime field on the player entity. `HealthDeathSystem` consults this field at damage-intent intake: any intent targeting an actor with `invulnerableUntilSimMs > simTimeMs` is dropped before damage hooks run, before HP changes, and before `hit`/`death` consequences. The field is **not** a status effect — it does not stack, does not animate, does not produce an event of its own. Future invulnerability use cases (pause-ghost, boss invincibility frames) reuse this primitive instead of adding a new layer.
