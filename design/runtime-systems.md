...
- Run completion (win/loss) is owned by the same `SessionFlowSystem`:
  ...
  - the `'allPlayersDead'` lossCondition kind ([session-definition.md](session-definition.md)) is recorded for online co-op (story 037) and not implemented yet; its consumer story owns implementation;
...
- Dynamic roster (story 036, [sim-core-interface.md](sim-core-interface.md), [session-definition.md](session-definition.md)):
  ...
  - `addPlayer.opts.invulnerableUntilSimMs`, when set, becomes a runtime field on the player entity. **`CombatSystem` consults this field at the same target-resolution stage that already excludes the projectile owner, pierce-deduplicated targets, and players defeated this tick** (collision target selection for projectile impact, explosion AoE eligibility, proximity detonation, and any non-player firing path that targets players): an actor with `invulnerableUntilSimMs > simTimeMs` is **not considered a valid target**. Consequence: no `hit` event, no `explosion`-derived hit event, no `DamageIntent`, no HP change for that actor; the projectile continues by its normal rules (pass-through, pierce decrement, ttl) as if the actor were not in the way. The filter is **not** in `HealthDeathSystem` — placing it after `CombatSystem` produced `hit` would leak a visible "hit on invulnerable" presentation event with zero damage, which is a game-feel regression vs. the compact arena sim being retired in story 036. The field is **not** a status effect — it does not stack, does not animate, does not produce an event of its own. Future invulnerability use cases (pause-ghost, boss invincibility frames) reuse this primitive and the same `CombatSystem` filter instead of adding a new layer.
