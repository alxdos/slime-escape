...
- The Socket.IO event for input intent stays `publicArena:input` (renamed at the protocol-rename step in story 036; the event name on the wire is recorded with the protocol types). Each intent is shaped as a shared `InputCommand` ([input-commands.md](input-commands.md)) plus the envelope fields described below.
...
- **Input sequence tracking (story 038, [online-client-prediction.md](online-client-prediction.md))**: every online input intent on the wire carries `inputSequence: number` ([input-commands.md](input-commands.md)). The host validates the sequence — it must be strictly greater than the last sequence applied for the same `actorId` in the active room — and drops out-of-order intents with a warning through the shared log module ([logging.md](logging.md)); intents that pass validation are forwarded into the core as usual. The host records the **last applied** `inputSequence` per `actorId` in per-room state and includes it in every snapshot through `Snapshot.lastInputSequence: { [playerId]: number }` ([snapshot-shape.md](snapshot-shape.md)). The map carries one entry per actor with at least one applied input; just-joined actors with no input yet have no entry. The map is rebuilt from the room's per-actor counters at every snapshot pass — there is no incremental delta on the wire. On `core.removePlayer(actorId)` the host drops the corresponding entry from the next snapshot's map; on `core.addPlayer(playerConfig)` the new actor enters the map only after its first input is applied. Rate-limited (dropped) intents do not advance `lastInputSequence` for that actor, which means the predictor's input buffer for those sequences will eventually be classified as "server-rejected" via the predicted-fire-rejected timeout in [online-client-prediction.md](online-client-prediction.md) — accepted as the natural consequence of the rate limit.
- **Spawn-input echo for predicted projectile reconciliation (story 038)**: when `core.submitInput(actorId, fireCommand)` is forwarded for a `fire` intent that produces a `Projectile` (cooldown ready, actor alive and not invulnerable, etc.), the runtime `Projectile.spawnInputSequence` is set to the originating `inputSequence` of the fire intent at the call site inside `CombatSystem`. `SnapshotExportSystem` copies the field into `ProjectileSnapshot.spawnInputSequence`. Projectiles whose origin is not a sequenced fire intent (slime/boss/companion fire, fragment cascades from a parent projectile) carry `spawnInputSequence: null`. The host does not need to maintain a separate per-actor mapping for this — the sequence threading happens inside the core and the snapshot copy is mechanical. `ProjectileSnapshot.ownerId` (also additive in 038) carries the `EntityId` of the owner so a fanned-out snapshot disambiguates "which player's projectile is this" without per-recipient reconstruction.
