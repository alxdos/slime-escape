...
- В фазе `DropSystem` тика (после `HealthDeathSystem.removeDead()`, до `ZoneSystem`) система проходит по всем дропам в `EntityStore` и принимает решения в строго заданном порядке:
  ...
  2. **pickup detection**: для дропов, не помеченных на удаление по ttl, проверить overlap с игроком: `dist(player.position, drop.position) <= player.radius + drop.radius`, где `player.radius` является derive-метрикой от спрайта игрока (`max(contactBox.width, contactBox.height) / 2`), а не ручным полем `content/players.md`. Если игрок отсутствует (`store.player() === null`) — фаза pickup для всех дропов no-op (это согласовано с общей толерантностью систем к `player === null` из [health-and-death.md](health-and-death.md)).
  3. при overlap: применить `drop.effect` (см. ниже), сообщить `RunSummaryTracker` о pickup-факте для итоговой статистики ([session-result-summary.md](session-result-summary.md)), пометить дроп на удаление, опубликовать `dropPickup`.
