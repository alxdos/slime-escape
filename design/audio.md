...
1. **Runtime events** (fan-out from `UiShell`):
   ...
   - `hit` -> `enemies[archetypeId].hit` or `bosses[archetypeId].hit`; target `archetypeId` is found through the current snapshot by `event.targetId`. **Player-with-form rule (story 037)**: if `event.targetKind === 'player'` and the resolved entity has `formArchetypeId !== null` ([snapshot-shape.md](snapshot-shape.md)), the audio resolver treats that `formArchetypeId` as the archetype id and plays `bosses[formArchetypeId].hit` when the form is the PvP boss form (id matches the active session's PvP boss archetype), otherwise `enemies[formArchetypeId].hit`. If `formArchetypeId === null` (canonical hero — local play and co-op), the resolver skips: heroes do not have a hit sample.
   - `death` -> `enemies[archetypeId].death` / `bosses[archetypeId].death`; target `archetypeId` for `entityKind === 'enemy' | 'boss'` is found through the current snapshot by `event.entityId`. **Player-with-form rule (story 037)**: identical to `hit` above — if `entityKind === 'player'` and the resolved entity has `formArchetypeId !== null`, play the form's `enemies`/`bosses` death sample; if `formArchetypeId === null` (canonical hero), skip (in single-player a hero death is followed by `loss` and the result overlay sound, not by a slime-style death sample). Server-side fan-out for `death` is already "every connected actor" per [online-session-hosting.md](online-session-hosting.md), so under this rule every PvP player audibly hears every other player fall in their slime form.
