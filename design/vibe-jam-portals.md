...
- A return portal is available only when an inbound portal context is active.
- While the context is active, each started session may show a return portal near the player spawn, including sessions started after a pre-boss loss and return to menu.
- The preferred return portal position is:
  ```text
  x = session.player.position.x - 2 * session.player.contactBox.width
...
- The return portal collapses and the inbound context is consumed when a boss encounter starts. After this boss lock-in, no later session in the same tab shows the return portal.
- Return portals are not shown during `boss` or `portal` encounters.

### Final portal encounter

- Add encounter type `portal` to `EncounterType`.
- A `portal` encounter is a non-combat terminal presentation encounter:
  - `spawnPlan.kind` is `empty`;
  - `zoneBehavior.kind` is `disabled`;
  - `transitionRules.kind` is `never`;
...
- The special `/portal` session ends its authored combat chain as:
  ```text
  waves -> boss-gargoyle -> portal encounter
  ```
- The portal session's completion path is external redirect, not a normal `win` result. It should use `winCondition: none` unless a later story explicitly adds a post-redirect result flow.

### Exit portal

- An exit portal is shown only in the `portal` encounter of the `/portal` session.
- The preferred exit portal position is the original player spawn point from `SessionDefinition.player.position`.
...
- When an exit portal is visible, no return portal is visible.
- Entering the exit portal redirects to:
  ```text
...
- Portal redirects are one-shot. Once a portal has triggered a redirect, repeated frames must not trigger more redirects.
...
- Portal outline is thick and animated between purple and lime.
- The same visual language is used for return and exit portals. Their meaning comes from placement and encounter timing.
...
- The final `/portal` encounter can keep the simulation running without a normal win result while the player chooses to step into the exit portal.
