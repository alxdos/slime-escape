...
- `Online combat affordances` are a desktop-only Public Arena layer owned by `UiShell`. It uses the standard movement hint, fire hint, and weapon-slot presentation from [hud-presentation.md](hud-presentation.md), with the online loadout containing exactly the shared `rock-thrower` weapon for this slice. It must not introduce separate online-only weapon-slot DOM or weapon switching rules.
