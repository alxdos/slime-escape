...
- Mobile controls are visible only in `running`. They are hidden in every modal or non-running phase.
- The mobile top-center pause/menu button routes through `UiShell` and opens the same `paused` phase as desktop pause inputs.
