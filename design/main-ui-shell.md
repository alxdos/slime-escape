...
- In story 029, when creating `Renderer`, `UiShell` also passes the currently selected pet id from `ClientProgressionStore`. This selected pet id is presentation configuration for renderer only. It is not included in `SessionDefinition`, `SimWorkerHost.startSession`, snapshots, or runtime events.
...
- Local progression is explicitly main-thread/browser state. XP and pet collection can enrich menu/result/renderer presentation without breaking simulation determinism while pets have no combat effect.
