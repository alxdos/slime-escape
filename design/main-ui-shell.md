...
- Presentation phase is a finite state machine:
  ...
  - `online`: Public Arena has accepted the socket. Online presentation is driven by server snapshots and the Public Arena client from [online-arena-hosting.md](online-arena-hosting.md), not by a local `SessionDefinition` or the one-player `SimWorkerHost` session runtime.
...
- Allowed transitions; any other transition is an orchestration bug, not UX freedom:
  ...
  - `loading -> onlineConnecting` after successful startup preload when a page entrypoint requests Public Arena online auto-start with `window.SLIME_ESCAPE_AUTO_START_PUBLIC_ARENA === true` and a build-time arena server URL is configured. This path uses the same Public Arena client connection flow as the explicit menu action, preserves the startup overlay/loading image before the transition, and must not build a local `SessionDefinition`, create the local `Renderer`/`InputController` path, or call `SimWorkerHost.startSession`.
  ...
  - `online -> menu` when the player chooses `Exit Arena` from the Public Arena menu, or when the server closes the connection;
...
| `online` | hidden | hidden | visible until online presentation is active | visible when opened by Public Arena menu input | visible on desktop when Public Arena menu is closed, hidden on mobile | online arena HUD when mounted | hidden | hidden | hidden | hidden | hidden | requested on desktop unless menu is open, released on mobile |
