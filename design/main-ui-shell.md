...
- Status: accepted
- Created: 2026-04-20
- Updated: 2026-05-01 (story 039 prep: online co-op renderer path now reads predicted snapshot pairs from the simulation worker. The shared `Renderer` (`src/main/render/Renderer.ts`) accepts an optional `getPredictedSnapshotPair` accessor; when wired (online sessions only), self-player and own in-flight projectiles are interpolated from the predicted snapshot pair, all other entities from the authoritative `SnapshotPair` with interpolation per [simulation-timing.md](simulation-timing.md). When omitted (local-authoritative play — campaign, training, dungeon, sandbox), the renderer's input/output is unchanged. `UiShell.ensureOnlineCampaignRenderer` wires the predicted accessor through; local-session renderer creation omits the parameter. The full prediction contract is in [online-client-prediction.md](online-client-prediction.md). Closes the gap left by story 037: the predictor was running in the worker for online sessions but its output was never read by the campaign-shape online renderer, so co-op players experienced full-RTT lag for self-movement. Earlier: 2026-05-01 story 037 prep: introduce `onlineLobby` phase between `onlineConnecting` and `online` for sessions whose content has `lobby.kind: 'hostControlled'` per [online-lobby.md](online-lobby.md). The online catalog becomes data-driven from session content (`# Session.online === true` per [content-authoring.md](content-authoring.md)) — the existing hardcoded "Public Arena" entry is generalised to one of N online modes including online co-op (story 037). When the chosen online session has a host-controlled lobby, `onlineConnecting → onlineLobby` (joiners wait, see roster + current host + host-only Start/Transfer/Leave controls); the host triggering start moves the room to `running` server-side, and the client transitions `onlineLobby → online` after the next `host:lobby:start` event. When the chosen online session has no lobby (Public Arena PvP), the existing `onlineConnecting → online` direct path is preserved. The `online` phase gains a co-op HUD variant: when the active online session emits a `Snapshot` carrying `encounter`/`waveProgress`/`bossHud`/`zone` (campaign-shape sessions), the existing presentation layers from local `running` (`Hud`, `EscapePath`, `TitleOverlay`) are mounted on top of the online combat path; PvP-shape sessions still use only the `PublicArenaHud`. Online runs reach `result(win|loss)` for the first time: when an online snapshot stream emits a terminal `win`/`loss` event with `SessionResultSummary` ([session-result-summary.md](session-result-summary.md)), `UiShell` enters `result(win|loss)` and Result UI renders the same way as for local sessions. Companion identity is sent on join: `selectedPetId` from `ClientProgression` accompanies the join handshake so the Node host can build the per-actor companion config ([online-session-hosting.md](online-session-hosting.md)). Earlier: 2026-04-30 story 032 T23 follow-up: `/portal` may request Public Arena online auto-start after startup preload, producing `loading -> onlineConnecting` without `autoStartPresetId`, session building, or `SimWorkerHost.startSession`; legacy local configured entrypoints may still use `autoStartPresetId` for `loading -> running`. Earlier story 032 T19 follow-up: desktop Public Arena online play mounts the standard combat affordance layer with movement hint, fire hint, and the normal weapon slot bar fed by the generated `portal` session loadout; mobile online keeps the mobile controls instead. Earlier story 032 follow-up: desktop `Esc`/`Space`/Pointer Lock loss and the mobile menu button open a minimal Public Arena menu during `online`; only choosing `Exit Arena` closes the socket and returns to `menu`. Earlier story 032 follow-up: Public Arena online play may mount the same mobile combat controls in `online` that local sessions mount in `running`, while still sending only the existing movement/aim/fire input intent. Earlier story 032: Public Arena adds a separate online phase branch through [online-session-hosting.md](online-session-hosting.md) (formerly `public-multiplayer-arena.md`, renamed in story 036). Story 031 follow-up: mobile combat uses the visible-area camera from [camera-and-visible-area.md](camera-and-visible-area.md) rather than changing the session arena; earlier story 031 prep: `UiShell` mounts player-facing game UI under a game root for mobile rotation, chooses desktop vs mobile input, and owns the mobile pause/control layer; see [mobile-web-support.md](mobile-web-support.md). Earlier: 2026-04-29 story 030 prep: `UiShell` passes selected pet into the session builder so companion-enabled runs receive `SessionDefinition.companion`; renderer no longer gets a hidden selected-pet gameplay path. Earlier: 2026-04-28 story 029 prep: `UiShell` owns local client progression for XP, owned pets, selected pet, Lab/Pets menu sub-screens, result XP award, and selected-pet renderer setup; pets remain presentation-only and do not enter sim. Earlier: story 028 prep: the existing Dungeon menu screen starts the `dungeon` preset, displays a localStorage-backed best wave number, and passes Dungeon best/new-best state into Result UI; no auth, leaderboard, or anti-cheat is added. Earlier: 2026-04-27 story 027 prep: loss results may show a loss-only Restart action; `UiShell` handles it by clearing result state through the menu cleanup path and starting the last session source through the normal start path, without introducing a public `result -> running` transition. Earlier: story 026 prep: the already implemented configured entrypoint auto-start is recorded as a sanctioned `loading -> running` path after successful preload when `UiShell` receives `autoStartPresetId`. Earlier: story 025: a separate UI layer `Escape Path` is added for compact/break progress path; its phase visibility, z-order, and result integration are defined in [escape-progress-path.md](escape-progress-path.md). Earlier story 024: Result UI consumes authoritative `SessionResultSummary` from terminal `win`/`loss` events and renders stats/effects without talking to sim; see [session-result-summary.md](session-result-summary.md). Earlier story 023: startup/menu presentation, post-load ritual, first-menu UI asset preload, and phase transition curtain are split into [menu-and-startup-presentation.md](menu-and-startup-presentation.md); this file keeps phase ownership and visibility. Earlier story 022: detailed combat HUD moved to [hud-presentation.md](hud-presentation.md); this file keeps only phase visibility, HUD passivity, and `UiShell` ownership. Earlier story 021: a separate `wave/break title overlay` UI layer is added; phase visibility, z-order, data sources, and global wave numbering are defined in [encounter-presentation.md](encounter-presentation.md). Earlier: 2026-04-24 017 terminology cleanup: session authoring references ordered `loadoutWeaponIds` instead of legacy singular `loadoutWeaponId`. Story 016: `UiShell` fans simulation runtime events out to `Renderer.handleEvent` while staying the only `SimWorkerHost.onEvent` owner; see [impact-feedback.md](impact-feedback.md). Earlier: story 015 follow-up: `Renderer` receives immutable `SessionDefinition` at creation, reads `session.backgrounds` + `encounters[].backgroundId`, and switches arena background by `snapshot.encounter.id`; snapshot is not extended because active encounter already exists. Story 015: playable preset catalog becomes a derived view over `SESSION_PRESET_TEMPLATES` from the content library; preset metadata (`displayName`/`description`/`visibleInMenu`/`order`) lives in the `# Session` partition of each `content/sessions/<presetId>.md`, next to the preset itself (see [content-authoring.md](content-authoring.md), "Multi-file areas"); separate `playableModes.ts` disappears; `PlayableModeEntry` collapses into a derived view with no separate registry. Story 013: startup phase `loading` and terminal phase `error('preload')` are added: visible splash plus sprite preload before menu, hard error on load failure, and no Renderer/InputController/Settings/SimWorkerHost availability in those phases.)
...
- Presentation phase is a finite state machine:
  ...
  - `onlineConnecting`: Socket.IO handshake to the Node host is in progress after an explicit online-mode action. No local session is active, `SimWorkerHost.startSession` has not been called, and the local campaign/training/dungeon renderer/input path is not created.
  - `onlineLobby` (story 037, host-controlled-lobby online sessions only — see [online-lobby.md](online-lobby.md)): the host has accepted the socket, the room is in `open` state, the player is waiting for the lobby host to trigger start. Lobby UI shows the joined roster, the current host actor, and host-only controls (Start, Transfer host to ▼, Leave). No simulation rendering yet — the room has not called `core.start(session)`. PvP sessions skip this phase entirely (their join goes `onlineConnecting → online`).
  - `online`: the Node host has accepted the socket and the session is `running`. Online presentation is driven by server snapshots and the online host client from [online-session-hosting.md](online-session-hosting.md), not by a local `SessionDefinition` or the one-player `SimWorkerHost` session runtime. Co-op sessions (campaign-shape snapshots) mount the standard combat HUD layers (`Hud`, `EscapePath`, `TitleOverlay`) on top of the online presentation path; PvP sessions continue to use only `PublicArenaHud`.
...
- Allowed transitions; any other transition is an orchestration bug, not UX freedom:
  ...
  - `loading -> running` after successful startup preload when `UiShell` receives a valid `autoStartPresetId` from a local page entrypoint. `UiShell` still builds a normal `SessionDefinition`, creates `Renderer`/`InputController`, calls `SimWorkerHost.startSession`, attaches HUD/presentation layers, and follows the same setup path as `menu -> running`. The menu must not become visible to the player on this path.
  - `loading -> onlineConnecting` after successful startup preload when a page entrypoint requests an online auto-start with `window.SLIME_ESCAPE_AUTO_START_PUBLIC_ARENA === true` and a build-time arena server URL is configured. This path uses the same online host client connection flow as the explicit menu action, preserves the startup overlay/loading image before the transition, and must not build a local `SessionDefinition`, create the local `Renderer`/`InputController` path, or call `SimWorkerHost.startSession`. The auto-start mechanism may evolve to carry a `requestedSessionId` once more than one online mode has a portal entrypoint — until then it implicitly targets `public-arena`.
  ...
  - `menu -> onlineConnecting` through the explicit online-mode action when a build-time arena server URL is configured. The chosen online mode determines `requestedSessionId` sent in the join handshake;
  - `onlineConnecting -> onlineLobby` after the server accepts the join into a host-controlled-lobby room (`host:lobby:state` arrives with `state: 'open'`);
  - `onlineConnecting -> online` after the server accepts the join into a no-lobby session (Public Arena PvP) or directly into a `running` co-op room with `lateJoinAllowed: true`;
  - `onlineConnecting -> menu` if the server rejects the join, the room is full, the socket closes before acceptance, or the player backs out;
  - `onlineLobby -> online` after the lobby host triggers start and the client receives the `host:lobby:start` event for its room;
  - `onlineLobby -> menu` when the player chooses `Leave` from the lobby UI, the server closes the connection, or the room is destroyed;
  - `online -> menu` when the player chooses `Exit Arena` from the online menu, or when the server closes the connection;
  - `online -> result(win|loss)` when an online snapshot stream emits a terminal `win`/`loss` event carrying `SessionResultSummary` (story 037 first; reachable for any online session whose `winCondition`/`lossCondition` produces terminal events);
...
| Phase | Startup overlay | Menu overlay | Public arena status | Public arena menu | Online combat affordances | HUD | Escape progress path | Title overlay | Pause overlay | Result UI | Startup error | Pointer Lock |
|------|-----------------|--------------|---------------------|-------------------|----------------------------|-----|----------------------|---------------|---------------|-----------|---------------|--------------|
| `loading` | visible | hidden | hidden | hidden | hidden | hidden | hidden | hidden | hidden | hidden | hidden | released |
| `menu` | hidden | visible | hidden | hidden | hidden | hidden | hidden | hidden | hidden | hidden | hidden | released |
| `onlineConnecting` | hidden | hidden | visible | hidden | hidden | hidden | hidden | hidden | hidden | hidden | hidden | released |
| `onlineLobby` | hidden | hidden | visible (lobby roster, host indicator, host-only Start/Transfer controls, Leave) | hidden | hidden | hidden | hidden | hidden | hidden | hidden | hidden | released |
| `online` | hidden | hidden | visible until online presentation is active | visible when opened by online menu input | visible on desktop when online menu is closed, hidden on mobile | online HUD when mounted (PvP-shape `PublicArenaHud` for PvP sessions; campaign-shape `Hud`/`EscapePath`/`TitleOverlay` mounted for co-op sessions whose snapshots carry `encounter`/`waveProgress`/`bossHud`/`zone`) | visible for co-op sessions whose snapshots carry it, hidden otherwise | visible for co-op sessions whose snapshots carry it, hidden otherwise | hidden | hidden | hidden | requested on desktop unless menu is open, released on mobile |
| `running` | hidden | hidden | hidden | hidden | hidden | visible (live) | visible (see [escape-progress-path.md](escape-progress-path.md)) | visible (see [encounter-presentation.md](encounter-presentation.md)) | hidden | hidden | hidden | requested |
| `paused` | hidden | hidden | hidden | hidden | hidden | visible (frozen on last snapshot) | visible (frozen) | visible (frozen) | visible | hidden | hidden | released |
| `result(win|loss)` | hidden | hidden | hidden | hidden | hidden | hidden | hidden (static path inside Result UI) | hidden | hidden | visible | hidden | released |
| `error('preload')` | hidden | hidden | hidden | hidden | hidden | hidden | hidden | hidden | hidden | hidden | visible | released |
...
- `Public arena menu` is a separate online-only UI layer owned by `UiShell`. Opening it does not call `SimWorkerHost.pause()` and does not make the local `paused` phase authoritative for multiplayer. While it is open, `UiShell` stops local online input and sends neutral movement/fire intent to the server. `Resume` hides the menu and restores online input; `Exit Arena` closes the Public Arena socket and returns to `menu`.
- `Online combat affordances` are a desktop-only Public Arena layer owned by `UiShell`. It uses the standard movement hint, fire hint, and weapon-slot presentation from [hud-presentation.md](hud-presentation.md), with slots fed by the generated `portal` session loadout. Current content has one selected `rock-thrower` slot. It must not introduce separate online-only weapon-slot DOM or weapon switching rules.
...
- Desktop Public Arena combat affordances are hidden in mobile `online`. Mobile `online` keeps the mobile movement/aim/fire controls from [mobile-web-support.md](mobile-web-support.md) and does not mount the desktop `WASD`, mouse-fire, or bottom weapon-slot bar.
- The mobile top-center pause/menu button routes through `UiShell`. In `running`, it opens the same `paused` phase as desktop pause inputs. In `online`, it opens the Public Arena menu. Choosing `Exit Arena` from that menu closes the Public Arena socket; tapping the mobile menu button itself does not leave multiplayer.
...
- In desktop `online`, `Esc`, `Space`, and Pointer Lock loss route through `UiShell` to open the Public Arena menu. They do not close the Public Arena socket directly, do not open `PauseOverlay`, do not call `SimWorkerHost.pause()`, and do not use the local-session pause path.
...
- Online sessions form a separate, data-driven **online catalog** alongside the local `playableModeCatalog`. The source of truth is the same `SESSION_PRESET_TEMPLATES` registry, filtered by an `online === true` field on each `# Session` partition (see [content-authoring.md](content-authoring.md)). Public Arena PvP (`presetId: 'public-arena'`) is one entry; online co-op (story 037) is another. Each entry carries the same `displayName`/`description`/`order` metadata as local entries; the online flag is what routes the menu choice through `menu -> onlineConnecting` instead of the session builder + `SimWorkerHost.startSession`. The hardcoded "Public Arena" menu button is replaced by an iteration over the online catalog. None of these entries call the session builder, `SimWorkerHost.startSession`, local campaign XP, pet progress, or Dungeon best-wave logic; selection sends `requestedSessionId: <presetId>` plus `selectedPetId` (from `ClientProgression`) in the join handshake to the Node host ([online-session-hosting.md](online-session-hosting.md)). Online runs may reach the local Result UI after a terminal `win`/`loss` event (see "Result UX" below); they still must not write to local progression / Dungeon best.
- The `/portal` page may request the same Public Arena online flow immediately after startup preload. It sets `window.SLIME_ESCAPE_AUTO_START_PUBLIC_ARENA = true`, not `SLIME_ESCAPE_AUTO_START = 'portal'`, and follows the `loading -> onlineConnecting` transition above. The generated `portal` session content remains available as a shared content source for Public Arena presentation/loadout data, but `/portal` does not execute that local session as a `ModePreset` run.
