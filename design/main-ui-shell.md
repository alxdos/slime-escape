...
- Allowed transitions; any other transition is an orchestration bug, not UX freedom:
  ...
  - `online -> menu` when the player exits the Public Arena or the server closes the connection;
...
| `online` | hidden | hidden | visible until online presentation is active | online arena HUD when mounted | hidden | hidden | hidden | hidden | hidden | released |
