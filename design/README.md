...
| [non-player-firing.md](non-player-firing.md) | accepted | Firing path для `kind: 'enemy'`: где живёт `WeaponInstance` слайма (на сущности), наивный aim в текущую позицию игрока, под-цикл фазы 1 `CombatSystem` в порядке EntityId, инициализация cooldown, cleanup на смерти, единый damage-rule helper для impact и explosion |
