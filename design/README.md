...
| [sprite-assets.md](sprite-assets.md) | accepted | Sprite-визуал для `player`/`enemy`/`boss`: `SpriteVisualSpec`, три раздельных visual registry рядом с renderer, `PX_PER_WU = 70`, asset-only renderer без circle-fallback, hard-error policy, preload до меню |
