...
| [online-client-prediction.md](online-client-prediction.md) | accepted | Source-style client-side prediction for online sessions: predict local player movement, own projectile spawn and motion, and movement-affecting status; reactive form snap on `host:levelUp`/snapshot `formArchetypeId` change; predictor runs as a separate `createOnlinePredictionCore` runtime built from shared sim systems in the existing browser worker; `inputSequence`/`lastInputSequence`/`spawnInputSequence`/`PlayerSnapshot.statusEffects` on the wire; snap-on-event vs blend-on-drift reconciliation; non-authoritative invariant |
