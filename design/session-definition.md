...
- Multi-actor `lossCondition` semantics (the three player-related kinds):
  ...
  - `'allPlayersDead'` — `loss` fires only when **every** player in the session is dead simultaneously. Recorded for online co-op (story 037) and not implemented yet; its consumer story owns implementation and tests.
