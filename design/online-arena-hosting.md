...
- The engine primitive is `addPlayer.opts.invulnerableUntilSimMs` ([sim-core-interface.md](sim-core-interface.md)). It is a single runtime field on the player entity consulted by `CombatSystem` at target-resolution stage (alongside the existing owner / pierce-deduplicated / defeated-this-tick filters); invulnerable actors are not considered valid targets, so no `hit`/`explosion`/`DamageIntent` is produced. Not a status effect.
