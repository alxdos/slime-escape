...
- The decision covers visual representation of entities `kind: 'player'`, `kind: 'enemy'`, `kind: 'boss'`, `kind: 'projectile'`, `kind: 'drop'`, runtime `kind: 'companion'`, and pet presentation through PNG sprites. All six visual registries render through the same `SpriteVisualSpec` contract and the same reference scale `PX_PER_WU`.
...
- Pet visual identity belongs to `PetArchetype.id` from the content library. In story 029 selected companion movement was renderer-owned presentation; in companion-enabled sessions after story 030, movement/state come from `CompanionSnapshot` and [companion-combat.md](companion-combat.md).
...
- There are six separate visual registries, one per presentation area. Each is a pair of handwritten consumer and generated literals, by [content-authoring.md](content-authoring.md):
  ...
  - pet: `src/main/render/petVisuals.ts` <-> `src/main/render/petVisuals.generated.ts`.
...
- Each handwritten module contains:
  - public type `SpriteVisualSpec`, physically owned in one shared place such as `src/main/render/SpriteVisualSpec.ts` and re-exported as needed. The contract is one project-wide type, not one copy per visual registry;
  - public registry `Readonly<Record<string, SpriteVisualSpec>>`;
  - validator that every id from the matching content registry has a visual spec and vice versa: `validatePlayerVisuals(playersRegistry)`, `validateEnemyVisuals(enemyRegistry)`, `validateBossVisuals(bossRegistry)`, `validateProjectileVisuals(weaponsRegistry)`, `validateDropVisuals(dropsRegistry)`, `validatePetVisuals(petsRegistry)`. Mismatch throws; it is not a warning.
- Generation sources by content area:
  ...
  - `content/pets.md` -> `petVisuals.generated.ts` (key is `petArchetypeId`).
...
- In `pets`, each pet H2 section carries a required inline image node for the pet sprite. `sourceSizePx` and `worldSize` are derived from that PNG; `anchor` remains fixed at `{0.5, 0.5}`.

### Asset-only renderer

- In [src/main/render/Renderer.ts](../src/main/render/Renderer.ts), branches for `player`/`enemy`/`boss`/`projectile`/`drop` and pet/companion sprites create `THREE.Mesh` with `THREE.PlaneGeometry(worldSize.width, worldSize.height)` and `THREE.MeshBasicMaterial({ map, transparent: true, depthWrite: false })`. `THREE.CircleGeometry` is not used for these kinds.
...
- Gameplay body contact for `player` / `enemy` / `boss` / `companion` does not live in this file. After [body-contact-boxes.md](body-contact-boxes.md), its owner is `contactBox` in the shared/runtime layer. `worldSize` and `contactBox` are separate contracts: the first is presentation-only, the second is gameplay. Companion `contactBox` is session-owned after story 030 and must not be derived from the selected pet sprite.
...
- On the current horizon the effect applies to `enemy`, `boss`, and pet/companion sprites. `player` stays unscaled so input feel and player silhouette remain stable; `projectile` carries its own render-only motion (spin, grounded pulse) governed by `WeaponArchetype.projectile.visual` in [universal-weapons-and-projectiles.md](universal-weapons-and-projectiles.md), and `drop` may carry render-only pulse/bob driven by `DropArchetype` content fields when added. Neither uses the procedural breathing curve described below.
- The breathing curve is sinusoidal and deterministic from render time plus entity identity:
  ```ts
  breath = sin(nowMs * breathHz + phaseKey * phaseStride)
  scaleX = 1 + amplitude * breath
  scaleY = 1 - amplitude * verticalRatio * breath
  ```
  Positive `breath` makes the slime wider and slightly lower; negative `breath` makes it narrower and taller. Runtime enemies/bosses use `entity.id` as `phaseKey`; pet/companion sprites use a stable hash or index derived from `petArchetypeId`. Different keys provide phase offsets so a wave of slimes does not animate in lockstep.
...
- Story 029 allowed a selected pet companion to be created only by `Renderer` from main-thread progression state. That renderer-only path is superseded for companion-enabled sessions.
- In story 030, if `SessionDefinition.companion` is present, `Renderer` creates and updates the pet mesh from `CompanionSnapshot` only. Runtime position, HP, alive/ghost state, mode, rescue progress, and target id are simulation outputs.
- If no `CompanionSnapshot` exists, no runtime companion mesh exists. A selected pet id in progression is not enough for the renderer to invent gameplay presentation.
- The renderer validates that `CompanionSnapshot.petArchetypeId` exists in the pet content registry, the `petVisuals` registry, and the preloaded texture set. Missing ids or textures are hard errors, not silent fallback.
- The companion uses the same procedural breathing curve style as enemies, with renderer-owned amplitude/timing constants. Look-around flips, ghost shimmer, rescue flip rate, and other personality details are renderer-owned and may read `petArchetypeId`, `state`, `mode`, and `rescueProgress`.
- Companion mesh z-order is near friendly/player presentation and must not hide projectiles, HUD, or player input feedback. Exact z values are renderer implementation details.
...
- The following situations are `throw` on startup/test, not silent fallback:
  ...
  5. A selected pet id from client progression or `CompanionSnapshot.petArchetypeId` has no pet content entry, pet visual spec, or preloaded texture.
- Checkpoints:
  - Validators (`validatePlayerVisuals`/`validateEnemyVisuals`/`validateBossVisuals`/`validateProjectileVisuals`/`validateDropVisuals`/`validatePetVisuals`) run at startup/session start, before first use, and catch cases 3 and 4.
...
- No `?? 0xffffff` colors, `?? CircleGeometry`, or `?? defaultTexture` for any snapshot sprite kind or pet/companion sprite. Any such fallback is a review blocker.
...
- The full `loading` phase, splash, and preload lifecycle are defined in [main-ui-shell.md](main-ui-shell.md). This file only records sprite-asset requirements:
  - The texture preload list is the union of `image` values from all six visual registries (`player`/`enemy`/`boss`/`projectile`/`drop`/`pet`), sorted for determinism.
...
- `validateEnemyVisuals` / `validateBossVisuals` / `validatePlayerVisuals` / `validateProjectileVisuals` / `validateDropVisuals` / `validatePetVisuals`: live registries pass happy path; mismatch (orphan visual / orphan archetype) throws with a clear message containing `archetypeId` and area name.
- Renderer hard error: creating a mesh for an `archetypeId` missing from visual registry throws; missing texture in the preloaded set throws. Regression test: `Renderer` does not create `CircleGeometry` for any snapshot entity kind or pet/companion sprite; any import/use of `CircleGeometry` in those branches is an error.
...
- Renderer breathing: `enemy` and pet/companion sprites receive render-only `mesh.scale` squash/stretch for a given `nowMs`, while `player` stays at `scale = 1`. `projectile` and `drop` do not receive the slime breathing curve; projectile scale may still come from `ProjectileSnapshot.size` and grounded pulse, and drop scale may come from renderer-owned pickup readability pulse.
- Runtime companion rendering: with `CompanionSnapshot`, renderer positions the pet mesh from snapshot `x/y`, draws alive/ghost/rescue presentation from companion fields, and does not run the old renderer-only follow movement for companion-enabled sessions. With no companion snapshot, no runtime companion mesh is created.
...
- 013 gets a compact contract: one shared `SpriteVisualSpec` type, three initial registries later extended to six, one `PX_PER_WU = 240` constant, and one hard-error path. Renderer stops being the place where magic drawing colors and radii live.
...
- Pets add one more preloaded visual registry. Story 030 widens the simulation snapshot through `CompanionSnapshot`; presentation still reads sprite identity from `petVisuals` and keeps pet-specific differences cosmetic.
