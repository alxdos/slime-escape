...
A predicted own projectile is **server-rejected** when both:

- its `spawnInputSequence` is acknowledged (`spawnInputSequence <= snapshot.lastInputSequence[selfPlayerId]`); **and**
- no authoritative `ProjectileSnapshot` for the local actor still carries that `spawnInputSequence` in the current snapshot (the set-based check from "Reconciliation" step 2).
